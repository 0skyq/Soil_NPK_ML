...
class UI:
    ...
    def setup_results_tab(self):
        ...
        self.canvas_frame = self.canvas.create_window((0, 0), window=self.soil_data_keys_frame, anchor="nw")
        ...
        # Ensure the canvas scrolls properly when contents change
        def on_frame_configure(event):
            self.canvas.configure(scrollregion=self.canvas.bbox("all"))

        self.soil_data_keys_frame.bind("<Configure>", on_frame_configure)

        # Also update the width if needed
        ...
        self.fetch_soil_data_button = tk.Button(
            self.result_frame, text="Fetch Soil Data Keys",
            command=self.process_handler.show_soil_data_keys,
            font=("Arial", 12)
        )
